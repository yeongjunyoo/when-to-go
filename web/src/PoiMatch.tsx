...
export default function PoiMatch({ attractionName, poiIndex, sidoName, signguName }: Props) {
  ...
  if (!poiIndex) {
    return (
      <div role="status" className="mt-3 rounded-md border border-default bg-surface-sunken p-3 text-xs text-text-subtle">
        위치 정보를 불러오는 중…
      </div>
    );
  }
  ...
  if (!outcome || outcome.kind === "zero_candidates") {
    return (
      <div role="status" className="mt-3 rounded-md border border-default bg-surface-sunken p-3 text-xs text-text-muted">
  ...
  if (outcome.kind === "multiple_candidates") {
    return (
      <div role="status" className="mt-3 rounded-md border border-warn-border bg-warn-bg p-3 text-xs text-warn-fg">
  ...
  // single_match
  return (
    <div className="mt-3 rounded-md border border-info-border bg-info-bg p-3 text-xs text-info-fg">
      <p className="mb-1 font-semibold">📍 연결 추정: {outcome.poi.title}</p>
      <p className="mb-2 text-info-fg">
      ...
      {detail.status === "success" && (
        <div>
          ...
          {!detail.common?.overview && detail.images.length === 0 && <p className="text-text-subtle">상세 정보 없음</p>}
        </div>
      )}
    </div>
  );
